...
async def get_insights(
    ...
    try:
        # ✅ Use the new is_recovered column set by email_routes.py sync loop.
        # Fall back to the join-based calculation if the column doesn't exist yet
        # (i.e. migration hasn't been run) so the endpoint never crashes.
        recovered_result = await db.execute(
            text("""
            SELECT
                COUNT(*) FILTER (WHERE et.is_recovered = TRUE)  AS recovered,
                AVG(
                    CASE
                        WHEN et.is_recovered = TRUE
                         AND et.recovered_at IS NOT NULL
                         AND et.last_followup_sent_at IS NOT NULL
                        THEN EXTRACT(EPOCH FROM (et.recovered_at - et.last_followup_sent_at)) / 3600.0
                        ELSE NULL
                    END
                ) AS avg_hours
        ...
        # Graceful fallback: use join-based heuristic if migration not yet run
        try:
            ...
            frow = fallback_result.fetchone()
            if frow:
                recovered_conversations = int(frow[0] or 0)
                avg_reply_time_hours = round(float(frow[1] or 0.0), 1)
        except Exception:
            recovered_conversations = 0
            avg_reply_time_hours = 0.0
